                    ...
                    messages.map((msg) => (
                        <div key={msg.id} className="glass-panel p-4 rounded-xl shadow-sm relative group">
                            <div className="flex justify-between items-start mb-1">
                                <span className="font-bold text-nh-green text-sm">{msg.author || '익명'}</span>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-2">
                    <div className="flex gap-2">
                        <input
                            ...
                            placeholder="작성자"
                            className="w-1/3 px-4 py-3 rounded-xl border border-white/50 bg-white/80 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-nh-green shadow-sm text-sm"
                            maxLength={10}
                        />
                        <input
                            type="text"
                    ...
                    <button
                        type="submit"
                        disabled={isSubmitting || !newMessage.trim() || !author.trim()}
                        className="w-full py-3 bg-gradient-to-r from-nh-green to-emerald-600 text-white rounded-xl font-bold shadow-lg disabled:opacity-50 disabled:shadow-none flex items-center justify-center"
                    >
                        {isSubmitting ? (
